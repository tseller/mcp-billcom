...
export function registerQboAccountTools(server: McpServer, client: QboClient) {
  server.registerTool(
    "qbo_list_accounts",
    {
      description: "List active accounts (chart of accounts) from QuickBooks — this is how you find the account id every other QBO tool asks for. " +
      ...
      inputSchema: z.object({ ...listPaging(1000) }),
    },
    (args) =>
  ...
  server.registerTool(
    ...
    {
      description: "Get current balances for all bank and credit card accounts. Useful for reconciliation.",
