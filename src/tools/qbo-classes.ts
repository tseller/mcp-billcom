...
export function registerQboClassTools(
  ...
  server.registerTool(
    "qbo_list_classes",
    {
      description: "List QuickBooks Classes — the season tags (e.g. Fall, Spring, General/Year-round) used to attribute transactions to a season. Returns each class's id, name, fully-qualified name (which includes the parent for sub-classes), active flag and parent. Use this to resolve a season name to the class id that the write tools need.",
  ...
  server.registerTool(
    "qbo_create_class",
    {
      description: "Create a QuickBooks Class (a season tag such as 'Fall 2026'). Safe to re-run: if a class with that name already exists it is returned instead of creating a near-duplicate, because QuickBooks happily accepts two classes with confusingly similar names and merging them afterwards is a manual chore in the web UI. Pass parentClassId to nest it under an existing class (e.g. seasons under a fiscal year).",
      inputSchema: z.object({
      name: z
        .string()
        .min(1)
